...
//        txtContent.setMovementMethod(ScrollingMovementMethod.getInstance()); //實現滾動
        ...
        final TtsRequest Intro = TtsRequest.create(Name.trim()+"位於"+Zone+"區"+floor.trim()+" 掃描 QR Code 可以取得地圖資訊", false);
        final TtsRequest Intro2 = TtsRequest.create(Name.trim()+"位於"+Zone+"區"+floor.trim()+" 按下立即前往 即可 為您帶位", false);
